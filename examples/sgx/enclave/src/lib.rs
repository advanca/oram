...
pub extern "C" fn say_something(some_string: *const u8, some_len: usize) -> sgx_status_t {
    ...
    println!(
        "gd: {} {} {} {} ",
        gd.get_static_tcs_num(),
        gd.get_eremove_tcs_num(),
        gd.get_dyn_tcs_num(),
        gd.get_tcs_max_num()
    );
    ...
    println!(
        "static: {} eremove: {} dyn: {}",
        static_num, eremove_num, dyn_num
    );

    unsafe {
        println!(
            "EDMM: {}, feature: {}",
            EDMM_supported, g_cpu_feature_indicator
        );
    }
    ...
    // Run oram example
    println!("running oram - in memory");
    oram::example_in_memory();

    println!("running oram - on disk");
    oram::example_on_disk(false);

    sgx_status_t::SGX_SUCCESS
}

#[no_mangle]
pub extern "C" fn reopen_db() -> sgx_status_t {
    println!("running oram - reopen db on disk");
    oram::example_on_disk(true);
...
fn get_thread_num() -> (u32, u32, u32) {
    ...
    unsafe {
        traversal_layout(
            &mut static_thread_num,
            &mut dyn_thread_num,
            &mut eremove_thread_num,
            layout_table,
        );
    }
